...
def extract_minutiae(image, isEndpoint, isBipoint, path):
    ...
    #cv2.imwrite('sm.jpg', smooth)
    #dispImg, result, minutiae = feature_extractor(smooth, isEndpoint, isBipoint, path)
    #return curve, result, dispImg, minutiae


if __name__ == "__main__":
    targetdir = r'C:\Users\CVlab\Documents\01_2023\01_2023_BiosyntheticData\hand_dorsal\0213_handdorsal_generate_sample'
    files = os.listdir(targetdir)
    for file in files:
        print(targetdir+file)
        path = targetdir + file
        img1 = cv2.imread('0213_handdorsal_generate_sample/'+file)
        #img1 = cv2.cvtColor(img1, cv2.COLOR_GRAY2RGB)
        #img1 = cv2.imread('M570801-FV3LNO1_ori.bmp')
        #print(img1.shape)# 0 for grayscale
        extract_minutiae(img1, False, True, path)
